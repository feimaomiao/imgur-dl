...
class link_grabber:
	...
	def __init__(self, link, verbose=False):
		...
		print('Video id search finished,\nlink: {}'.format(video_id_search.group(0))) if self.verbose else None
		...
		try:
			...
			print('Video id get successful!\nVideo id: {}'.format(self._albumextension)) if self.verbose else None

		except AttributeError:

			print('Video id is not in expected imgur format.\nNo file is expected') if self.verbose else None
			# Raised when the user did not enter an imgur link
			raise ValueError('Please input a proper link!') from AttributeError

		print('Getting blog link from video id {{}}\nConnecting to website'.format(self._albumextension)) if self.verbose else None

		# get the longer link
		self.__actuallink = req_get("http://imgur.com/a/" + self._albumextension + "/layout/blog")

		print('Connecting to blog link:\nBlog link: {url}\nStatus code: {status}'.format(url=self.__actuallink.url,status=self.__actuallink.status_code)) if self.verbose else None
...
if self.__actuallink.status_code==404 else '' ))
		...
		print('Connected to blog...\nLooking for references to images/videos') if self.verbose else None

		# find hash and extension
		self._reflist = re.findall(r'.*?{"hash":"([a-zA-Z0-9]+)".*?"ext":"(\.[a-zA-Z0-9]+)".*?', self.__actuallink.text)

		print('Found {length} matched cases, shrinking duplicated lists'.format(length=len(self._reflist))) if self.verbose else None
		...
		print('Getting unique values was successful, {} items in total'.format(self._supp_len)) if self.verbose else None
	...
	def return_file_objects(self):
		...
		files = []
		for count, links in progress_bar(enumerate(self.generate_download_links()), unit='connections', total=self._supp_len, desc='Loading images'):
			try:
				req_obj = imgur_object(link=links, verbose=self.verbose)
				files.append(req_obj)
				if count == 0 and self.verbose:  clear_lines(1)

			except NetworkError as ne:

				print('NetworkError is raised in link {}'.format(count)) if self.verbose else None
				print(ne)
		...
		print('Final list length is {length}, expencted length is {explength}.\nSuccessful rate is {percentage}'.format(length=len(files), explength=self._supp_len,percentage=(str(len(files)/self._supp_len*100))+'%' )) if self.verbose else None
